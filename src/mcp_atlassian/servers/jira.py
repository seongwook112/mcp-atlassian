...
async def get_all_project_members(
    ...
    all_members_map: dict[str, dict[str, str]] = {}  # Use accountId as key to ensure uniqueness

    try:
        project_roles = jira.get_project_roles(project_key)
        if not project_roles:
            logger.warning(f"No roles found for project {project_key} or project does not exist.")
            # Return empty list if project has no roles or project itself not found by get_project_roles
        ...
        for role_name, role_url in project_roles.items():
            try:
                ...
                if not role_id.isdigit():
                    logger.warning(f"Could not extract a valid role_id from URL: {role_url} for role: {role_name}")
                    continue
                ...
                for member in role_members:
                    ...
                    actor_type = member.get("type") # type can be atlassian-group-role-actor, atlassian-user-role-actor etc.

                    # We are interested in individual users
                    if account_id and display_name and actor_type == "atlassian-user-role-actor":
                ...
                logger.error(
                    f"Error getting members for role '{role_name}' (ID: {role_id}) in project {project_key}: {str(e_inner)}"
                )
        ...
        logger.error(f"Failed to get all members for project {project_key}: {str(e)}", exc_info=True)
        error_payload = {
            "success": False,
